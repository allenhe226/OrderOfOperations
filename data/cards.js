...
export function generateCard(round) {
    ...
    if (op === "add") {
        ...
        return {type: type, label: `+${amount}`, fn: v => v + amount};
    }
    if (op === "sub") {
        const amount = randomInt(1, Math.round(1 + power * 3));
        return {type: type, label: `-${amount}`, fn: v => v - amount};
    ...
    if (op === "mul") {
        ...
        return {type: type, label: `×${factor}`, fn: v => v * factor};
    }
    if (op === "div") {
        const factor = randomInt(2, Math.max(2, Math.round(1 + power * 0.5)));
        return {type: type, label: `/${factor}`, fn: v => Math.round(v / factor)};
    }
    if (op === "exp") {
        const exponent = randomInt(2, Math.max(2, Math.round(2 + power * 0.2)));
        return {type: type, label: `^${exponent}`, fn: v => v**exponent}
    }
    if (op === "root") {
        const exponent = randomInt(2, Math.max(2, Math.round(2 + power * 0.2)));
        return {type: type, label: `√${exponent}`, fn: v => Math.round(v**(1.0/exponent))}
    }
}
